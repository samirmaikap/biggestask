...
export const HomeScreen = () => {
  ...
  const shouldShowWeeklyUpdate = state.weeklyUpdate && state?.weeklyUpdate?.id;
        ...
        <View style={styles.innerContainer}>
            ...
            <AppCard padding={16}>
              <View style={[styles.row, {justifyContent: 'space-between'}]}>
                <View style={{marginRight: 16, flex: 1}}>
                  ...
                  <AppSpacing gap={8} />
                  <AppText>{state.weeklyUpdate.description}</AppText>
                </View>
                <View>
                  <AppImage
                    size={50}
          ...
          {alertQuestions?.length > 0 &&
            alertQuestions.map((item: any, index: any) => {
              return (
                <View key={`alert-${index}`} style={{marginVertical: 16}}>
                  <AppCard padding={16}>
                    <AppText variant="h2">
                      {item?.question_id === 246
                        ? `${item?.question.text.replace('?', ' ')}for ${
                          item?.milestone.name
                        }?`
                        : item?.question.text}
                    </AppText>
                    ...
                    <AppButton
                      onPress={() =>
                        navigation.navigate(Screens.MilestoneDetails, {
                          activeMilestoneId: item?.milestone_id,
                        })
                      }
              ...
              <SkeletonPlaceholder borderRadius={4}>
                <SkeletonPlaceholder.Item
                  ...
                  <SkeletonPlaceholder.Item
                    ...
                    borderRadius={50}
                  />
                  <SkeletonPlaceholder.Item marginBottom={20} marginTop={20}>
          ...
          {state.nextMilestone && state.nextMilestone?.id && (
            <TouchableOpacity
              ...
              <AppCard padding={16}>
                ...
                <AppText variant="h2">{state.nextMilestone?.name}</AppText>
                <AppSpacing />
                <View style={styles.row}>
                  ...
                  <AppText variant={'custom'} size={12} color={Colors.grey_3}>
                    {nextMilestoneDate
                      ? getLocalDateTime(nextMilestoneDate, state.timeZone)
                      : 'Not yet scheduled'}
                  </AppText>
                </View>
                <AppSpacing gap={16} />
