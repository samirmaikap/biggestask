...
export const MilestoneDetailsScreen = () => {
  ...
  useEffect(() => {
    if (activeMilestoneId) {
      ...
      if (m) {
        ...
        if (m.parsed_date_time) {
          setDate(m.parsed_date_time);
        }
  ...
  const handleMilestoneSave = async () => {
    const payload = {
      journey_id: activeMilestone?.journey_id
        ...
        : state.journey?.id,
      name: title,
      date_time: date ? getUTCDateTime(date, state.timeZone) : null,
    ...
    toast.show(activeMilestone?.id ? 'Milestone updated' : 'Milestone created');
            ...
            <View style={[styles.row, {alignItems: 'flex-start'}]}>
              ...
              <View>
                <AppText color={Colors.grey_2}>
                  {date
                    ? getLocalDateTime(date, state.timeZone)
                    : 'Not yet scheduled'}
                </AppText>
          ...
          <View>
            ...
            <TouchableOpacity
              ...
              <View
                ...
                <AppText>
                  {date ? getLocalDateTime(date, state.timeZone) : ''}
                </AppText>
              </View>
            </TouchableOpacity>
            <AppSpacing gap={16} />
            ...
            <AppSpacing gap={16} />
            {state.user?.user_type === 'parent' && activeMilestoneId && (
              <View style={[styles.row, {alignItems: 'center'}]}>
                <Switch
                  onValueChange={v => handleRequestDate(v)}
                  value={requestDate}
                  style={{
                    transform: [
                      {
  ...
  return (
    <View style={styles.container}>
      <StackHeader
        title={activeMilestone?.id ? 'Edit Milestone' : 'Add New Milestone'}
      />
      <KeyboardAvoidingView
          ...
          <View style={styles.innerContainer}>
            ...
            {!hasImage && (
              <View style={[styles.uploadContainer, styles.centeredContainer]}>
                <View style={styles.centeredContainer}>
                  <AppText>No image available</AppText>
                  <AppSpacing gap={8} />
                  <AppText
                    textAlign={'center'}
                    variant={'caption'}
                    color={Colors.grey_3}>
                    Upload a picture to share with your surrogacy partner
                  </AppText>
                </View>
                <AppSpacing gap={16} />
                <View style={styles.row}>
            ...
            {otherNote && (
              <View>
                <AppText variant={'title'}>
                  Note from {isSurrogate ? 'Parents' : 'Gestational Carrier'}
                </AppText>
                <AppSpacing />
            ...
            {state.user?.user_type === 'parent' ? (
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => setShareWithSurrogate(!shareWithSurrogate)}
                style={styles.row}>
                <Checkbox.Android
                  status={shareWithSurrogate ? 'checked' : 'unchecked'}
                  onPress={() => {
                    setShareWithSurrogate(!shareWithSurrogate);
                  }}
                />
                <AppSpacing isHorizontal={true} />
                <AppText>Share with Gestational Carrier</AppText>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
                activeOpacity={0.8}
                ...
                style={styles.row}>
                <Checkbox.Android
                  status={shareWithParent ? 'checked' : 'unchecked'}
                  onPress={() => {
            ...
            <TouchableOpacity
              ...
              style={styles.row}>
              <Checkbox.Android
                status={shareBiggestask ? 'checked' : 'unchecked'}
                onPress={() => {
            ...
            <AppButton
              ...
              style={AppStyles.button}>
              {activeMilestone?.id ? 'Update Milestone' : 'Create Milestone'}
            </AppButton>
      ...
      <DatePicker
        ...
        onConfirm={(d: any) => {
          console.log('d', getUTCDateTime(d, state.timeZone));
